...
///
/// Used by [NewDropdownButton.selectedItemBuilder].
...
class _DropdownMenuItemButtonState<T>
  ...
  void _handleOnTap() {
    final NewDropdownMenuItem<T> dropdownMenuItem =
        widget.route.items[widget.itemIndex].item!;

    dropdownMenuItem.onTap?.call();

    if (widget.route.isSingleChoice != null && widget.route.isSingleChoice!) {
      Navigator.pop(
        context,
  ...
  Widget build(BuildContext context) {
    ...
    if (dropdownMenuItem.enabled) {
      child = InkWell(
        ...
        child: Container(
          padding: widget.padding,
          child: widget.route.items[widget.itemIndex].item!.childBuilder!(//todo null check
            context,
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final NewDropdownMenuItem<T>? item;
...
class _DropdownMenuItemContainer<T> extends StatelessWidget {
  ...
  final Widget Function(
          BuildContext context, dynamic currentValue)? //todo error with T ???
      childBuilder;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      alignment: alignment,
      child: child ?? childBuilder!(context, value),
    );
  }
}

/// An item in a menu created by a [NewDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class NewDropdownMenuItem<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  NewDropdownMenuItem({
  ...
  ///
  /// Eventually returned in a call to [NewDropdownButton.onChanged].
  final T? value;
  ...
  NewDropdownMenuItem<T> copyWith({
    VoidCallback? onTap,
    T? value,
    Widget? child,
  }) {
    return NewDropdownMenuItem<T>(
...
/// An inherited widget that causes any descendant [NewDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [NewDropdownButton] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [NewDropdownButton] widgets should
  /// be hidden.
...
/// Typically, an enum is used. Each [NewDropdownMenuItem] in [items] must be
...
/// See also:
///
///  * [NewDropdownMenuItem], the class used to represent the [items].
///  * [DropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [ElevatedButton], [TextButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class NewDropdownButton<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [NewDropdownMenuItem] values. If [items] or
  ...
  NewDropdownButton({
    ...
    // DropdownButtonFormField.
          ...
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((NewDropdownMenuItem<T> item) {
  ...
  final List<NewDropdownMenuItem<T>>? items;

  /// The value of the currently selected [NewDropdownMenuItem].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [NewDropdownButton.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [NewDropdownButton.disabledHint] widget if it is non-null.
  /// If [NewDropdownButton.disabledHint] is also null but [NewDropdownButton.hint] is
  /// non-null, [NewDropdownButton.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [NewDropdownMenuItem]s in [items].
  ///
  /// When a [NewDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [NewDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [NewDropdownMenuItem].
  ///
  /// ```dart
  ...
  /// {@end-tool}
  ///
  /// If this callback is null, the [NewDropdownMenuItem] from [items]
  ...
  @override
  State<NewDropdownButton<T>> createState() => _NewDropdownButtonState<T>();
}

class _NewDropdownButtonState<T> extends State<NewDropdownButton<T>>
  ...
  var dataStream = StreamController<T>.broadcast();
  ...
  void initState() {
    ...
    if (widget.value != null) {
      dataStream.sink.add(widget.value!);
    }
  }
  ...
  @override
  void didUpdateWidget(NewDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null ||
        ...
        (widget.value == null &&
            widget.items!
                .where((NewDropdownMenuItem<T> item) =>
    ...
    assert(widget.items!
            .where((NewDropdownMenuItem<T> item) => item.value == widget.value)
            .length ==
        1);
  ...
  void _handleTap() {
    ...
    final List<_MenuItem<T>> menuItems = <_MenuItem<T>>[
      for (int index = 0; index < widget.items!.length; index += 1)
        _MenuItem<T>(
          item: widget.items?[index].copyWith(onTap: () {
            widget.items?[index].onTap?.call();
            // if(widget.items?[index].value != null) {
            dataStream.sink.add(widget.items![index].value!);
            // }
...
/// A convenience widget that makes a [NewDropdownButton] into a [FormField].
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [NewDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  DropdownButtonFormField({
    Key? key,
    required List<NewDropdownMenuItem<T>>? items,
